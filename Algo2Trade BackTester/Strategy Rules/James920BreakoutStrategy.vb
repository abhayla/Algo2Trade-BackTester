...
Public Class James920BreakoutStrategy
    ...
    Public Overrides Sub CalculateRule(ByRef outputPayload As Dictionary(Of String, Object))
        If _inputPayload IsNot Nothing AndAlso _inputPayload.Count > 0 Then
            ...
            Dim xMinutePayload As Dictionary(Of Date, Payload) = _cmn.ConvertPayloadsToXMinutes(_inputPayload, 5)
            Dim niftyOneMinutePayload As Dictionary(Of Date, Payload) = _cmn.GetRawPayload(Common.DataBaseTable.Intraday_Futures, "NIFTY", _inputPayload.LastOrDefault.Key.Date, _inputPayload.LastOrDefault.Key.Date)
            Dim niftyXMinutePayload As Dictionary(Of Date, Payload) = Nothing
            If niftyOneMinutePayload IsNot Nothing AndAlso niftyOneMinutePayload.Count > 0 Then
                niftyXMinutePayload = _cmn.ConvertPayloadsToXMinutes(niftyOneMinutePayload, 5)
            End If
            For Each runningPayload In _inputPayload.Keys
                ...
                Dim signalCheckTime As Date = New Date(runningPayload.Year, runningPayload.Month, runningPayload.Day, 9, 20, 0)

                If _inputPayload(runningPayload).PreviousCandlePayload IsNot Nothing AndAlso
                    runningPayload = signalCheckTime AndAlso xMinutePayload.ContainsKey(runningPayload) AndAlso
                    niftyXMinutePayload IsNot Nothing AndAlso niftyXMinutePayload.Count > 0 AndAlso
                    niftyXMinutePayload.ContainsKey(runningPayload) Then
                    Dim currentXMinuteCandle As Payload = xMinutePayload(runningPayload)
                    Dim currentXMinuteNiftyCandle As Payload = niftyXMinutePayload(runningPayload)
                    If currentXMinuteCandle IsNot Nothing AndAlso currentXMinuteCandle.PreviousCandlePayload IsNot Nothing AndAlso
                        currentXMinuteNiftyCandle IsNot Nothing AndAlso currentXMinuteNiftyCandle.PreviousCandlePayload IsNot Nothing Then
                        If (currentXMinuteCandle.PreviousCandlePayload.High - currentXMinuteCandle.PreviousCandlePayload.Open) > 0.012 * currentXMinuteCandle.PreviousCandlePayload.Open Then
                            potentialEntryPrice = currentXMinuteCandle.PreviousCandlePayload.High
                            potentialEntryPrice += CalculateBuffer(potentialEntryPrice, RoundOfType.Celing)
                            If currentXMinuteCandle.PreviousCandlePayload.CandleColor = Color.Green AndAlso
                                currentXMinuteNiftyCandle.PreviousCandlePayload.CandleColor = Color.Green AndAlso
                                _inputPayload(runningPayload).High >= potentialEntryPrice Then
                                signal = 1
                                entryPrice = potentialEntryPrice
                                slPrice = entryPrice - entryPrice * 0.01
                                targetPrice = entryPrice + entryPrice * 0.02
                            End If
                        ElseIf (currentXMinuteCandle.PreviousCandlePayload.Open - currentXMinuteCandle.PreviousCandlePayload.Low) > 0.012 * currentXMinuteCandle.PreviousCandlePayload.Open Then
                            potentialEntryPrice = currentXMinuteCandle.PreviousCandlePayload.Low
                            potentialEntryPrice -= CalculateBuffer(potentialEntryPrice, RoundOfType.Celing)
                            If currentXMinuteCandle.PreviousCandlePayload.CandleColor = Color.Red AndAlso
                                currentXMinuteNiftyCandle.PreviousCandlePayload.CandleColor = Color.Red AndAlso
                                _inputPayload(runningPayload).Low <= potentialEntryPrice Then
                                signal = -1
                                entryPrice = potentialEntryPrice
                                slPrice = entryPrice + entryPrice * 0.01
                                targetPrice = entryPrice - entryPrice * 0.02
