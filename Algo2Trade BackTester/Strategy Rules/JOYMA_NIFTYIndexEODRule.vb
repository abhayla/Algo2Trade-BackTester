...
Public Class JOYMA_NIFTYIndexEODRule
    Inherits StrategyRule
    Implements IDisposable

    Private _cmn As Common = Nothing
    Private ReadOnly _initialInvestment As Decimal = 60000
    Private _totalAvailableCapital As Decimal
    Private _tradingDate As Date
    Public Sub New(ByVal inputPayload As Dictionary(Of Date, Payload),
                   ByVal tickSize As Decimal, ByVal quantity As Integer,
                   ByVal canceller As CancellationTokenSource,
                   ByVal cmn As Common, ByVal previousPL As Decimal,
                   ByVal tradingDate As Date)
        MyBase.New(inputPayload, tickSize, quantity, canceller)
        _cmn = cmn
        _totalAvailableCapital = previousPL + _initialInvestment
    ...
    Public Overrides Sub CalculateRule(ByRef outputPayload As Dictionary(Of String, Object))
        If _inputPayload IsNot Nothing AndAlso _inputPayload.Count > 0 Then
            ...
            If _inputPayload.LastOrDefault.Key.Date = _tradingDate.Date Then
                ...
                Dim previousTradingDay As Date = _cmn.GetPreviousTradingDay(Common.DataBaseTable.EOD_Futures, tradingSymbol.Remove(tradingSymbol.Count - 8), _tradingDate)
                Dim eodPayload As Dictionary(Of Date, Payload) = _cmn.GetRawPayloadForSpecificTradingSymbol(Common.DataBaseTable.EOD_Futures, tradingSymbol, previousTradingDay, previousTradingDay)

                If eodPayload IsNot Nothing AndAlso eodPayload.Count > 0 Then
                    Dim previousDayPayload As Payload = eodPayload.FirstOrDefault.Value
                    ...
                    For Each runningPayload In _inputPayload.Keys
                        ...
                        Dim quantity As Integer = Strategy.CalculateQuantityFromInvestment(_quantity, _totalAvailableCapital / 2, _inputPayload.LastOrDefault.Value.Open, Trade.TypeOfStock.Futures)
                        ...
                        If quantity < 1 Then
                            Throw New ApplicationException(String.Format("Check quantity. Quantity: {0}, Investment:{1}, Price:{2}, Date:{3}", quantity, _totalAvailableCapital / 2, _inputPayload.LastOrDefault.Value.Open, _tradingDate.ToShortDateString))
                        End If
                        ...
                        If _inputPayload(runningPayload).PreviousCandlePayload IsNot Nothing AndAlso
                           ...
                           runningPayload.Date = _tradingDate.Date Then
                            ...
                            If previousDayPayload.CandleColor = Color.Red Then
                                signal = 1
                                entryPrice = _inputPayload(runningPayload).Ticks(randomNumber).Open
                                targetPrice = entryPrice + 100000
                                slPrice = entryPrice - Utilities.Numbers.ConvertFloorCeling(If(_inputPayload(runningPayload).Open >= previousDayPayload.Close, previousDayPayload.CandleRange, previousDayPayload.CandleRange / 2), _tickSize, Utilities.Numbers.NumberManipulation.RoundOfType.Floor)
                            ElseIf previousDayPayload.CandleColor = Color.Green Then
                                signal = -1
                                entryPrice = _inputPayload(runningPayload).Ticks(randomNumber).Open
                                targetPrice = entryPrice - 100000
                                slPrice = entryPrice + Utilities.Numbers.ConvertFloorCeling(If(_inputPayload(runningPayload).Open <= previousDayPayload.Close, previousDayPayload.CandleRange, previousDayPayload.CandleRange / 2), _tickSize, Utilities.Numbers.NumberManipulation.RoundOfType.Floor)
                            End If
                            supporting1 = previousDayPayload.CandleColor.ToString
                            supporting2 = previousDayPayload.High
                            supporting3 = previousDayPayload.Low
                            supporting4 = _inputPayload(runningPayload).Open
                            supporting5 = _totalAvailableCapital
