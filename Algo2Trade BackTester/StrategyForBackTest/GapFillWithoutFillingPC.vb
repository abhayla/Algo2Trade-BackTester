...
Public Class GapFillWithoutFillingPC
    Inherits Strategy
...
label1:                     Next
        ...
        Dim filename As String = String.Format("Gap Fill Without Filling PC Strategy PL-{0} Stocks-{1} SL-{2} {3}-{4}-{5}_{6}-{7}-{8}.xlsx",
            ...
            For Each stock In tempStocklist.Keys
                ...
                If intradayPayload IsNot Nothing AndAlso intradayPayload.Count > 0 Then
                    ...
                    If tempStocklist(stock)(0) = 1 Then
                        If previousHigh < eodPayload.LastOrDefault.Value.Close Then
                    ...
                    ElseIf tempStocklist(stock)(0) = -1 Then
                        If previousLow > eodPayload.LastOrDefault.Value.Close Then
                            If ret Is Nothing Then ret = New Dictionary(Of String, Double())
                            ret.Add(stock, tempStocklist(stock))
                            count += 1
                        End If
