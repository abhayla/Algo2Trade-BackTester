...
Public Class GapFillWithoutFillingPCNeglectGnG
    Inherits Strategy
    Implements IDisposable

    Dim capitalPerStock As Double = Nothing
    Dim totalPL As Double = Nothing
...
label1:                     Next
        ...
        Dim filename As String = String.Format("Gap Fill Without Filling PC Neglect GnG Strategy PL-{0} Stocks-{1} SL-{2} {3}-{4}-{5}_{6}-{7}-{8}.xlsx",
        ...
        If tempStocklist IsNot Nothing AndAlso tempStocklist.Count > 0 Then
            ...
            For Each stock In tempStocklist.Keys
                ...
                If intradayPayload IsNot Nothing AndAlso intradayPayload.Count > 0 Then
                    Dim previousMaxHigh As Decimal = intradayPayload.Max(Function(x)
                    ...
                    Dim previousMinLow As Decimal = intradayPayload.Min(Function(x)
                                                                            If x.Key < potentialCandleSignalTime Then
                                                                                Return x.Value.Low
                                                                            Else
                                                                                Return Decimal.MaxValue
                    ...
                    Dim previousPlus1MaxClose As Decimal = intradayPayload.Max(Function(x)
                                                                                   If x.Key > firstCandle.PayloadDate AndAlso x.Key < potentialCandleSignalTime Then
                                                                                       Return x.Value.Close
                                                                                   Else
                                                                                       Return Decimal.MinValue
                                                                                   End If
                                                                               End Function)

                    Dim previousPlus1MinClose As Decimal = intradayPayload.Min(Function(x)
                                                                                   If x.Key > firstCandle.PayloadDate AndAlso x.Key < potentialCandleSignalTime Then
                                                                                       Return x.Value.Close
                                                                                   Else
                                                                                       Return Decimal.MaxValue
                    ...
                    Dim previousPlus1MaxHigh As Decimal = intradayPayload.Max(Function(x)
                                                                                  If x.Key > firstCandle.PayloadDate AndAlso x.Key < potentialCandleSignalTime Then
                                                                                      Return x.Value.High
                                                                                  Else
                                                                                      Return Decimal.MinValue
                    ...
                    Dim previousPlus1MinLow As Decimal = intradayPayload.Min(Function(x)
                                                                                 If x.Key > firstCandle.PayloadDate AndAlso x.Key < potentialCandleSignalTime Then
                                                                                     Return x.Value.Low
                                                                                 Else
                                                                                     Return Decimal.MaxValue
                    ...
                    If tempStocklist(stock)(0) = 1 Then
                        If previousMaxHigh < eodPayload.LastOrDefault.Value.Close Then
                            If previousPlus1MaxHigh >= firstCandle.High Then
                                If ret Is Nothing Then ret = New Dictionary(Of String, Double())
                                ret.Add(stock, tempStocklist(stock))
                                count += 1
                            ElseIf previousPlus1MinClose >= firstCandle.Low Then
                                If ret Is Nothing Then ret = New Dictionary(Of String, Double())
                                ret.Add(stock, tempStocklist(stock))
                                count += 1
                            End If
                    ...
                    ElseIf tempStocklist(stock)(0) = -1 Then
                        If previousMinLow > eodPayload.LastOrDefault.Value.Close Then
                            If previousPlus1MinLow <= firstCandle.Low Then
                                If ret Is Nothing Then ret = New Dictionary(Of String, Double())
                                ret.Add(stock, tempStocklist(stock))
                                count += 1
                            ElseIf previousPlus1MaxClose <= firstCandle.High Then
                                If ret Is Nothing Then ret = New Dictionary(Of String, Double())
                                ret.Add(stock, tempStocklist(stock))
                                count += 1
                            End If
