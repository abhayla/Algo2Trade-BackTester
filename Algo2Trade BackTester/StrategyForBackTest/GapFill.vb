...
label1:                     Next
        ...
        Dim filename As String = String.Format("Gap Fill Strategy PL-{0} Stocks-{1} SL-{2} {3}-{4}-{5}_{6}-{7}-{8}.xlsx",
        ...
        If dts IsNot Nothing AndAlso dts.Tables.Count > 0 Then
            ...
            While Not count > totalTables - 1
                ...
                If temp_dt.Rows.Count > 0 Then
                    ...
                    temp_dt = temp_dt.Select("Abs>=1", "Abs DESC").Cast(Of DataRow).Take(NumberOfTradeableStockPerDay).CopyToDataTable
                End If
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
                    If dt.Rows(i).Item(7) > 1000 AndAlso (dt.Rows(i).Item(5) >= 1 OrElse dt.Rows(i).Item(5) <= -1) Then
                        Dim signalDirection As Integer = If(dt.Rows(i).Item(5) >= 1, -1, 1)
                        If outputPayload Is Nothing Then outputPayload = New Dictionary(Of String, Double())
                        'TODO:
                        'Change lot size to 1 for cash
