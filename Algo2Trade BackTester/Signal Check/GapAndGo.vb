...
#End Region
    ...
    Private Function GetModifiedStockData(databaseTableType As Common.DataBaseTable, tradingDate As Date) As Dictionary(Of String, Double())
        ...
        Dim firstSignalTime As TimeSpan = TimeSpan.Parse("09:20:00")
        Dim secondSignalTime As TimeSpan = TimeSpan.Parse("09:25:00")
        Dim potentialFirstSignalTime As Date = New Date(tradingDate.Year, tradingDate.Month, tradingDate.Day, firstSignalTime.Hours, firstSignalTime.Minutes, firstSignalTime.Seconds)
        ...
        If tempStocklist IsNot Nothing AndAlso tempStocklist.Count > 0 Then
            For Each stock In tempStocklist.Keys
                ...
                If intradayPayload IsNot Nothing AndAlso intradayPayload.Count > 0 Then
                    Dim firstMaxHigh As Decimal = intradayPayload.Max(Function(x)
                                                                          If x.Key < potentialFirstSignalTime Then
                                                                              Return x.Value.High
                                                                          Else
                    ...
                    Dim firstMinLow As Decimal = intradayPayload.Min(Function(x)
                                                                         If x.Key < potentialFirstSignalTime Then
                                                                             Return x.Value.Low
                                                                         Else
                    ...
                    Dim secondMaxHigh As Decimal = intradayPayload.Max(Function(x)
                    ...
                    Dim secondMinLow As Decimal = intradayPayload.Min(Function(x)
                                                                          If x.Key < potentialSecondSignalTime Then
                                                                              Return x.Value.Low
                                                                          Else
                                                                              Return Decimal.MaxValue
                    ...
                    If tempStocklist(stock)(0) = 1 Then
                        If secondMaxHigh > firstMaxHigh AndAlso firstCandle.Low <= secondMinLow Then
                    ...
                    ElseIf tempStocklist(stock)(0) = -1 Then
                        If secondMinLow < firstMinLow AndAlso firstCandle.High >= secondMaxHigh Then
                            If ret Is Nothing Then ret = New Dictionary(Of String, Double())
                            ret.Add(stock, tempStocklist(stock))
                        End If
                    End If
