                ...
                For Each runningPayload In _inputPayload.Keys
                    ...
                    If runningPayload.Date = _tradingDate.Date Then
                        If potentialHighEntryPrice = 0 AndAlso potentialLowEntryPrice = 0 Then
                            If _inputPayload(runningPayload).High <= Math.Round(ATRHighPayload(_inputPayload(runningPayload).PayloadDate), 2) AndAlso
                                _inputPayload(runningPayload).Low >= Math.Round(ATRLowPayload(_inputPayload(runningPayload).PayloadDate), 2) Then
                                signalCandle = _inputPayload(runningPayload)
                                If Me.CandleBasedEntry Then
