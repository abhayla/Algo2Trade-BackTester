...
Public Class ATRBandHedging
    ...
    Private ReadOnly _signalType As StrategyRule.TypeOfSignal = TypeOfSignal.Sell
    ...
    Public Sub New(ByVal inputPayload As Dictionary(Of Date, Payload),
        ...
        _signalStartTime = New Date(_tradingDate.Year, _tradingDate.Month, _tradingDate.Day, 9, 16, 0)
    End Sub
    ...
    Public Overrides Sub CalculateRule(ByRef outputPayload As Dictionary(Of String, Object))
        If _inputPayload IsNot Nothing AndAlso _inputPayload.Count > 0 Then
            ...
            If _inputPayload.LastOrDefault.Key.Date = _tradingDate.Date Then
                ...
                Indicator.ATRBands.CalculateATRBands(1, 5, Payload.PayloadFields.Close, _inputPayload, highATRBandPayload, lowATRBandPayload)
                ...
                For Each runningPayload In _inputPayload.Keys
                    ...
                    If runningPayload.Date = _tradingDate.Date Then
                        If runningPayload >= _signalStartTime Then
                            ...
                            If _inputPayload(runningPayload).PreviousCandlePayload.CandleColor = Color.Green Then
                                Dim lowATRBand As Decimal = lowATRBandPayload(_inputPayload(runningPayload).PreviousCandlePayload.PayloadDate)
                                If _inputPayload(runningPayload).PreviousCandlePayload.Open <= lowATRBand AndAlso
                                    _inputPayload(runningPayload).PreviousCandlePayload.Close >= lowATRBand Then
                                    takeTrade = True
                                    slPoint = ConvertFloorCeling(Math.Abs(_inputPayload(runningPayload).Open - lowATRBand), _tickSize, RoundOfType.Celing)
                                End If
                            ElseIf _inputPayload(runningPayload).PreviousCandlePayload.CandleColor = Color.Red Then
                                Dim highATRBand As Decimal = highATRBandPayload(_inputPayload(runningPayload).PreviousCandlePayload.PayloadDate)
                                If _inputPayload(runningPayload).PreviousCandlePayload.Open >= highATRBand AndAlso
                                    _inputPayload(runningPayload).PreviousCandlePayload.Close <= highATRBand Then
                                    takeTrade = True
                                    slPoint = ConvertFloorCeling(Math.Abs(_inputPayload(runningPayload).Open - highATRBand), _tickSize, RoundOfType.Celing)
                                End If
                            End If
                            If takeTrade AndAlso slPoint <> Decimal.MinValue Then
                                If _signalType = TypeOfSignal.Buy Then
                                    entryData.BuySignal = 1
                                    entryData.BuyEntry = _inputPayload(runningPayload).Open
                                    entryData.BuyStoploss = entryData.BuyEntry - slPoint
                                    ...
                                    End If
                                    supporting1 = _inputPayload(runningPayload).PreviousCandlePayload.Open
                                    supporting2 = _inputPayload(runningPayload).PreviousCandlePayload.Open
                                    supporting3 = highATRBandPayload(_inputPayload(runningPayload).PreviousCandlePayload.PayloadDate)
                                    supporting4 = lowATRBandPayload(_inputPayload(runningPayload).PreviousCandlePayload.PayloadDate)
                                    supporting5 = slPoint
                                ElseIf _signalType = TypeOfSignal.Sell Then
                                    entryData.SellSignal = -1
                                    entryData.SellEntry = _inputPayload(runningPayload).Open
                                    entryData.SellStoploss = entryData.SellEntry + slPoint
