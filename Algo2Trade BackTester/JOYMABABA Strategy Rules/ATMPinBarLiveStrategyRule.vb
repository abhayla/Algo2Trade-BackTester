...
Public Class ATMPinBarLiveStrategyRule
    ...
    Public Overrides Sub CalculateRule(ByRef outputPayload As Dictionary(Of String, Object))
        If _inputPayload IsNot Nothing AndAlso _inputPayload.Count > 0 Then
            ...
            If _inputPayload.LastOrDefault.Key.Date = _tradingDate.Date Then
                ...
                Dim tradeEnterd As Boolean = False
                ...
                For Each runningPayload In _inputPayload.Keys
                    ...
                    If runningPayload.Date = _tradingDate.Date Then
                        ...
                        If potentialHighEntryPrice = 0 OrElse potentialLowEntryPrice = 0 OrElse levelPrice = 0 Then
                            If runningPayload > _signalCandleTime Then
                                If IsSignalCandle(_inputPayload(runningPayload)).Item1 Then
                                    signalCandle = _inputPayload(runningPayload)
                                    If IsSignalCandle(signalCandle).Item2 = 1 Then
                                        potentialHighEntryPrice = signalCandle.High + CalculateBuffer(signalCandle.High, RoundOfType.Floor)
                                        levelPrice = potentialHighEntryPrice - ConvertFloorCeling(_usableATR * ATRMultiplier, _tickSize, RoundOfType.Celing)
                                        potentialLowEntryPrice = levelPrice - ConvertFloorCeling(_usableATR * ATRMultiplier, _tickSize, RoundOfType.Celing)
                                        firstSignal = 1
                                    ElseIf IsSignalCandle(signalCandle).Item2 = -1 Then
                                        potentialLowEntryPrice = signalCandle.Low - CalculateBuffer(signalCandle.Low, RoundOfType.Floor)
                                        levelPrice = potentialLowEntryPrice + ConvertFloorCeling(_usableATR * ATRMultiplier, _tickSize, RoundOfType.Celing)
                                        potentialHighEntryPrice = levelPrice + ConvertFloorCeling(_usableATR * ATRMultiplier, _tickSize, RoundOfType.Celing)
                                        firstSignal = -1
                                    End If
                                End If
                            End If
                        ElseIf potentialHighEntryPrice <> 0 AndAlso potentialLowEntryPrice <> 0 AndAlso levelPrice <> 0 Then
                            If _inputPayload(runningPayload).High >= potentialHighEntryPrice Then
                                ...
                                If firstSignal = 1 Then tradeEnterd = True
                            ...
                            If _inputPayload(runningPayload).Low <= potentialLowEntryPrice Then
                                ...
                                supporting2 = _usableATR
                            End If
                            If Not tradeEnterd Then
                                If firstSignal = 1 AndAlso _inputPayload(runningPayload).Low < signalCandle.Low Then
                                    signalCandle = Nothing
                                    potentialHighEntryPrice = 0
                                    potentialLowEntryPrice = 0
                                    levelPrice = 0
                                    firstSignal = 0
                                ElseIf firstSignal = -1 AndAlso _inputPayload(runningPayload).High > signalCandle.High Then
                                    signalCandle = Nothing
                                    potentialHighEntryPrice = 0
                                    potentialLowEntryPrice = 0
                                    levelPrice = 0
    ...
    Private Function IsSignalCandle(ByVal candle As Payload) As Tuple(Of Boolean, Integer)
        ...
        If candle IsNot Nothing AndAlso candle.PreviousCandlePayload IsNot Nothing Then
            ...
            '    End If
            'End If
            Dim wick As Decimal = Math.Abs(candle.CandleWicks.Top - candle.CandleWicks.Bottom)
            If candle.CandleWicks.Top > candle.CandleWicks.Bottom AndAlso (wick / candle.CandleRange) * 100 >= 50 Then
                If candle.High > candle.PreviousCandlePayload.High AndAlso
                   candle.Low > candle.PreviousCandlePayload.Low Then
                    ret = New Tuple(Of Boolean, Integer)(True, -1)
                End If
            ElseIf candle.CandleWicks.Top < candle.CandleWicks.Bottom AndAlso (wick / candle.CandleRange) * 100 >= 50 Then
                If candle.Low < candle.PreviousCandlePayload.Low AndAlso
                   candle.High < candle.PreviousCandlePayload.High Then
                    ret = New Tuple(Of Boolean, Integer)(True, 1)
                End If
