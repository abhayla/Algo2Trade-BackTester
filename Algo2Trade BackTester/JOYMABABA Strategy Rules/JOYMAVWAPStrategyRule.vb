...
Public Class JOYMAVWAPStrategyRule
    ...
    Private ReadOnly _maxStoplossPercentage As Decimal = 0.5
    Private ReadOnly _maxTargetPercentage As Decimal = 1
                ...
                For Each runningPayload In _inputPayload.Keys
                    ...
                    If runningPayload.Date = _tradingDate.Date Then
                        If Not firstCandleOfTradingDate Then
                            Dim VWAPValue As Decimal = VWAPPayload(_inputPayload(runningPayload).PreviousCandlePayload.PayloadDate)

                            If _inputPayload(runningPayload).PreviousCandlePayload.Open > VWAPValue AndAlso
                                _inputPayload(runningPayload).PreviousCandlePayload.Close > VWAPValue Then
                                If lastSignal = 0 OrElse lastSignal = -1 Then
                                    If _inputPayload(runningPayload).PreviousCandlePayload.Low <= VWAPValue Then
                                        supporting1 = _inputPayload(runningPayload).PreviousCandlePayload.PayloadDate.ToString
                                        supporting2 = VWAPValue
                                        potentialSignal = 1
                                        potentialHighEntryPrice = _inputPayload(runningPayload).PreviousCandlePayload.High
                                    ...
                                    End If
                                End If
                            ElseIf _inputPayload(runningPayload).PreviousCandlePayload.Open < VWAPValue AndAlso
                                _inputPayload(runningPayload).PreviousCandlePayload.Close < VWAPValue Then
                                If lastSignal = 0 OrElse lastSignal = 1 Then
                                    If _inputPayload(runningPayload).PreviousCandlePayload.High >= VWAPValue Then
                                        supporting1 = _inputPayload(runningPayload).PreviousCandlePayload.PayloadDate.ToString
                                        supporting2 = VWAPValue
                                        potentialSignal = -1
                                        potentialLowEntryPrice = _inputPayload(runningPayload).PreviousCandlePayload.Low
                            ...
                            If potentialSignal = 1 Then
                                If potentialHighEntryPrice <> 0 Then
                                    If _inputPayload(runningPayload).High >= potentialHighEntryPrice Then
                                        ...
                                        slPrice = entryPrice - entryPrice * _maxStoplossPercentage / 100
                                        targetPrice = entryPrice + entryPrice * _maxTargetPercentage / 100
                                        potentialSignal = 0
                                        potentialHighEntryPrice = 0
                                        lastSignal = 1
                                    Else
                                        'If _inputPayload(runningPayload).CandleColor = Color.Red Then
                                        supporting1 = _inputPayload(runningPayload).PayloadDate.ToString
                                        supporting2 = VWAPPayload(runningPayload)
